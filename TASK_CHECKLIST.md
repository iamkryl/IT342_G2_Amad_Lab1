## ✅ DONE
- Create project repository on GitHub [commit: 421fb26]
- Initialize Spring Boot backend (userauth) [commit: 421fb26]
- Create backend folder structure: model, repository, controller, service [commit: 421fb26]
- Create ReactJS frontend folder structure [commit: 421fb26]
- Create /docs folder [commit: 421fb26]
- Update README.md with project details [commit: 421fb26]
- Web Application (ReactJS) pages: Register, Login, Dashboard/Profile, Logout [commit: 6573215]
- Backend endpoints implemented: /api/auth/register, /api/auth/login, /api/auth/logout, /api/user/me [commit: 6573215]
- Documentation (FRS Partial) updated with ERD, UML diagrams, and web UI screenshots [commit: 6573215]
- Added LogoutRequest DTO and standardized logout endpoint [commit: 5f89763]
- Updated class diagram to include LogoutRequest DTO [commit: 5f89763]
- Create UI for login - profile pages for mobile app.[commit: 3c95f67]
- Finalize documents SRS [commit: 3c95f67]
